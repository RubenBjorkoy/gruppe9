...
const testAnswers: Svar[] = [
	{
		svarid: 1,
		svartekst: "Create a new database schema in MySQL Workbench.",
		poeng: 2,
		sporsmalid: 1,
		dato: roundedDate,
	...
	},
	{
		svarid: 2,
		svartekst:
			"Interfaces in TypeScript are similar to interfaces in Java and C#. They are used for strongly typing objects.",
		poeng: 1,
		sporsmalid: 2,
		dato: roundedDate,
		sistendret: roundedDate,
	...
	},
	{
		svarid: 3,
		svartekst:
			"Async/await is syntactic sugar for promises in JavaScript. It makes asynchronous programming easier to understand and write.",
		poeng: 3,
		sporsmalid: 3,
		dato: roundedDate,
		sistendret: roundedDate,
	...
	{
		svarid: 4,
		svartekst: "I really don't know either",
		poeng: -1,
		sporsmalid: 1,
		dato: roundedDate,
		sistendret: roundedDate,
	...
	test("Fetch all comments to a post (200 OK)", (done) => {
		axios.get("/sporsmal/1/svar").then((response) => {
			const expectedData = JSON.parse(
				JSON.stringify(testAnswers.filter((answer) => answer.sporsmalid === 1))
			);
			const receivedData = JSON.parse(JSON.stringify(response.data));
...
describe("Update comment (POST)", () => {
	test("Update comment (200 OK)", (done) => {
		...
		axios
			.put(`/sporsmal/${testAnswer.sporsmalid}/svar`, [testAnswer])
			.then((response) => {
				expect(response.status).toEqual(200);
				done();
			});
	...
	test("Delete comment with a reply (200 OK)", (done) => {
		...
		svarService.create(testAnswer).then((id) => {
			...
			svarService.create(testReply).then((id) => {
				axios
					.delete(
						`/sporsmal/${testAnswer.sporsmalid}/svar/${testReply.svarsvarid}`
					)
					.then((response) => {
						expect(response.status).toEqual(200);
						done();
