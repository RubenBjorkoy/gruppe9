...
beforeEach((done) => {
  ...
  pool.query('TRUNCATE TABLE SporsmalTag', (error) => {
    ...
    sporsmalTagService.createRelation(testSporsmalTag[0].sporsmalid, testSporsmalTag[0].tagid)
        .then(() => sporsmalTagService.createRelation(testSporsmalTag[1].sporsmalid, testSporsmalTag[1].tagid))
        .then(() => sporsmalTagService.createRelation(testSporsmalTag[2].sporsmalid, testSporsmalTag[2].tagid))
        .then(() => done())
...
describe('Fetch tags for a question (GET)', () => {
  test('Fetch tags for a question (200 OK)', (done) => {
    axios.get(`/sporsmal/${testSporsmalTag[0].sporsmalid}/tags`).then((response) => {
...
describe('Create SporsmalTag relation (POST)', () => {
  ...
  test('Fetch questions for a tag (400 Bad Request)', (done) => {
    ...
    axios.post(`/sporsmalTag`, errSporsmalTag).then(() => {
          ...
          .catch((error) => {
            ...
            expect(error.response.data).toEqual('Tag does not exist');
            done();
          });
    });
  });
...
describe('Delete SporsmalTag relation (DELETE)', () => {
  test('Delete SporsmalTag relation (200 OK)', (done) => {
    axios.delete(`/sporsmalTag/${testSporsmalTag[0].sporsmalid}/${testSporsmalTag[0].tagid}`).then((response) => {
      expect(response.status).toEqual(200);
      done();
    });
  });
