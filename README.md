...
POST /sporsmal
```
- **Description**: Creates a new question. ID is automatically generated by the database. sistendret is automatically set to the same value as dato which are both generated by the server. The bestsvarid is automatically set to null. The bestsvarid can be updated using the PUT endpoint. The ersvart is automatically set to false as there won't be generated an answer until a user posts one. The ersvart can be updated using the PUT endpoint.
- **Request Body**:
```json
{
  "sporsmal": {
