...
class SporsmalTagService {
    ...
    getTagForSporsmal(sporsmalid: number) {
        return axios.get<Tag[]>(`/sporsmal/${sporsmalid}/tags`).then((response) => response.data);
    }

    getSporsmalForTags(tagid: number) {
        return axios.get<Sporsmal[]>(`/tag/${tagid}/sporsmal`).then((response) => response.data);
    }


    create(sporsmalid: number, tagid: number) {
        return axios
          .post<{ tag: Tag }>('/sporsmal/', { sporsmalid: sporsmalid, tagid: tagid})
