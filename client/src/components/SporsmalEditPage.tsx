...
class SporsmalEdit extends Component<{
	...
	handleUpdate = async () => {
        ...
        await sporsmalService.update(updatedSporsmal, true, this.chosenTags).then(() => {
                                ...
                                <>
                                    <Row key={tag.tagid}>
                                        ...
                                        <Column width={5}>
                                            <Button.Danger
                                                onClick={() => {
                                                    sporsmalTagService.delete(this.props.match.params.sporsmalid, tag.tagid);
                                                    this.tags.push(tag);
                                                    this.chosenTags = this.chosenTags.filter(
                                                        (chosenTag) => {return chosenTag.tagid != tag.tagid}
                                                    );
                                                    this.tags.sort((a, b) =>
                                                        a.tagid - b.tagid
                                                    );
    ...
    componentDidMount() {
        ...
        sporsmalService.get(this.props.match.params.sporsmalid).then((sporsmal: Sporsmal) => {
